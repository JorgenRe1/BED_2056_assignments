...
# Recode the variable “region” into tidy names

# Is this not already done above? 


# Roomcap overtime per county and whole country

# Filter out only roomcap data

roomCaps <- dframe[dframe$variable1 == "roomcap",]

# Plot data

roomCaps %>%
  ggplot( aes(x=date, y=value, group=region, color=region)) +
  xlab(paste("Måned av", unique(roomCaps$year), sep=" ")) +
  ylab("Prosent") +
  labs(title=roomCaps$variable)+
  geom_line()
